...
class Helper:
  ...
  for idc in range(num_clients):
    ...
    for id_com_round in range(num_com_round):
      ...
      # CHOOSE K CLIENTS INSTEAD OF N
      K = 4
      random_clients = random.sample(flserver.clients, K)

      global_update_key = copy.deepcopy(global_weights)
      for _, key in enumerate(global_update_key.keys()):
        global_update_key[key] = global_weights[key] * (num_clients - K) / num_clients
      ...
      for flclient in random_clients:
      ...
      for _, key in enumerate(global_weights.keys()):
        global_weights[key].add_(global_update_key[key])
